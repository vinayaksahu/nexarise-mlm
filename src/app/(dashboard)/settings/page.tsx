...
export default function SettingsPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.profile) {
            ...
            setPayoutData({
              defaultPayoutMethod: data.profile.defaultPayoutMethod || 'CRYPTO',
  ...
  const handleUpdatePayoutMethods = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/user/profile', {
        ...
        body: JSON.stringify({
          ...payoutData,
          cryptoWalletAddress: payoutData.cryptoWalletAddress, // Also update legacy walletAddress for backwards compatibility
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto animate-fade-in">
      ...
      {/* Receiving Payment Methods (Crypto, Banking, UPI) */}
      <Card className="p-5 sm:p-6 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-200 dark:border-slate-800 pb-4">
          <div>
            ...
            <p className="text-xs text-muted mt-0.5">
              Set up your withdrawal receiving accounts (Crypto, Bank, UPI) and select your active default.
            </p>
          </div>
        ...
        <form onSubmit={handleUpdatePayoutMethods} className="space-y-6">
          ...
          <Button type="submit" className="w-full sm:w-auto px-6 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5" disabled={payoutLoading}>
