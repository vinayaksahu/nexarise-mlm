...
export default function AdminBusinessPlanPage() {
  ...
  const [config, setConfig] = useState<{
    ...
    minDirectReferralsForLevel: defaultLevelIncomes.map((_, i) => i + 1),
    requireSelfInvestmentForLevelIncome: false,
    requiredSelfInvestmentForLevel: defaultLevelIncomes.map(() => 0),
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.plan && data.plan.config) {
          ...
          const minDirects = Array.isArray(cfg.minDirectReferralsForLevel) && cfg.minDirectReferralsForLevel.length === count
            ? cfg.minDirectReferralsForLevel
            : Array.from({ length: count }, (_, i) => i + 1);

          const reqSelf = Array.isArray(cfg.requiredSelfInvestmentForLevel) && cfg.requiredSelfInvestmentForLevel.length === count
            ? cfg.requiredSelfInvestmentForLevel
            : Array.from({ length: count }, () => 0);

          const rawRewards = cfg.achievementRewards && Array.isArray(cfg.achievementRewards) && cfg.achievementRewards.length > 0
            ? cfg.achievementRewards
            : defaultAchievementRewards;

          const rewards = rawRewards.map((r: any) => ({
            name: r.name,
            volumeRequired: Number(r.volumeRequired || 0),
          ...
          setConfig({
            ...
            minDirectReferralsForLevel: minDirects,
            requireSelfInvestmentForLevelIncome: cfg.requireSelfInvestmentForLevelIncome ?? false,
            requiredSelfInvestmentForLevel: reqSelf,
            maxLevels: count,
  ...
  const handleAddLevel = () => {
    ...
    const newMinDirects = [...config.minDirectReferralsForLevel, nextNum];
    const newReqSelf = [...config.requiredSelfInvestmentForLevel, 0];

    setConfig({
      ...config,
      levelIncomePercentages: newLevels,
      minDirectReferralsForLevel: newMinDirects,
      requiredSelfInvestmentForLevel: newReqSelf,
  ...
  const handleRemoveLevel = () => {
    ...
    const newMinDirects = config.minDirectReferralsForLevel.slice(0, -1);
    const newReqSelf = config.requiredSelfInvestmentForLevel.slice(0, -1);
  ...
  const handleAddReward = () => {
    const newRewards = [
      ...
      { name: 'New Rank', volumeRequired: 100000, rewardAmount: 2500, isActive: true }
    ];
  ...
  const handleToggleRewardActive = (idx: number) => {
    const newRewards = [...config.achievementRewards];
    newRewards[idx] = {
      ...newRewards[idx],
      isActive: !(newRewards[idx].isActive !== false)
    };
    setConfig({ ...config, achievementRewards: newRewards });
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const levelQualifications = config.minDirectReferralsForLevel.map(num => formatDirectReferrals(num));
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted text-xs sm:text-sm mt-0.5">
            Configure ROI rates, referral level commissions, qualification requirements, required self-investments, and achievement reward milestones.
          </p>
        </div>
      </div>
      ...
      <Card className="p-6 max-w-4xl space-y-8">
        ...
        <div className="pt-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 border-b border-border pb-3">
            <div>
              <h2 className="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <span>2. 🏆 Level Referral Commission & Qualifications</span>
                <Badge variant="info" className="text-[10px]">{config.levelIncomePercentages.length} Levels</Badge>
              </h2>
              <p className="text-xs text-muted mt-0.5">Configure commission rates, required direct referrals, and optional self-investment per level.</p>
            </div>
          ...
          {/* Master Switch for Required Self Investment Column */}
          <div className="mb-4 p-3 bg-gray-50 dark:bg-slate-950 rounded-xl border border-gray-200 dark:border-slate-800 flex items-center justify-between">
            <div className="space-y-0.5">
              <span className="text-xs font-bold text-gray-900 dark:text-white">Enable "Required Self Investment" Column</span>
              <p className="text-[11px] text-muted">When enabled, admins can set required personal self investment ($) per level, displayed on public & user panel.</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer shrink-0">
              <input 
                type="checkbox" 
                checked={config.requireSelfInvestmentForLevelIncome} 
                onChange={e => setConfig({...config, requireSelfInvestmentForLevelIncome: e.target.checked})}
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-gray-300 peer-focus:outline-none rounded-full peer dark:bg-slate-800 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          </div>
            ...
            {config.levelIncomePercentages.map((perc, idx) => (
              <div key={idx} className="p-3 border rounded-xl bg-gray-50 dark:bg-slate-900/60 border-gray-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-1">
                  <label className="text-[11px] text-muted font-medium shrink-0">Direct Referrals Required:</label>
                  <div className="flex items-center gap-1.5 flex-1">
                    <Input 
                      type="number" min="0" step="1"
                      value={config.minDirectReferralsForLevel[idx] ?? (idx + 1)} 
                      onChange={e => {
                        const newDirects = [...config.minDirectReferralsForLevel];
                        newDirects[idx] = parseInt(e.target.value) || 0;
                        setConfig({...config, minDirectReferralsForLevel: newDirects});
                      }}
                      className="bg-white dark:bg-slate-950 border-gray-300 dark:border-slate-700 text-gray-900 dark:text-white text-xs font-bold w-20"
                    />
                    <span className="text-xs text-gray-500 dark:text-slate-400 font-medium">
                      Direct Referral{(config.minDirectReferralsForLevel[idx] ?? (idx + 1)) === 1 ? '' : 's'}
                    </span>
                  </div>
                </div>

                {config.requireSelfInvestmentForLevelIncome && (
                  <div className="flex items-center gap-2 sm:w-44 shrink-0 border-l border-gray-200 dark:border-slate-800 pl-3">
                    <label className="text-[11px] text-muted font-medium shrink-0">Self Investment:</label>
                    <div className="relative flex-1">
                      <span className="absolute left-2 top-2 text-xs text-gray-400 font-semibold">$</span>
                      <Input 
                        type="number" min="0" step="5"
                        value={config.requiredSelfInvestmentForLevel[idx] ?? 0} 
                        onChange={e => {
                          const newReqSelf = [...config.requiredSelfInvestmentForLevel];
                          newReqSelf[idx] = parseFloat(e.target.value) || 0;
                          setConfig({...config, requiredSelfInvestmentForLevel: newReqSelf});
                        }}
                        className="bg-white dark:bg-slate-950 border-gray-300 dark:border-slate-700 text-gray-900 dark:text-white text-xs font-bold pl-6"
                      />
                    </div>
                  </div>
                )}
        ...
        <div className="pt-2">
            ...
            <div>
              <h2 className="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                <Badge variant="info" className="text-[10px]">
                  {config.achievementRewards.filter(r => r.isActive !== false).length} Active / {config.achievementRewards.length} Total Ranks
                </Badge>
              </h2>
              <p className="text-xs text-muted mt-0.5">Hit business volume milestones across downline network (50% max per leg rule) to unlock cash rank bonuses.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {config.achievementRewards.map((reward, idx) => {
              const isRewardActive = reward.isActive !== false;
              return (
                <div
                  key={idx}
                  className={`p-4 border rounded-xl space-y-3 transition-all ${
                    isRewardActive
                      ...
                      : 'bg-gray-100/60 dark:bg-slate-950/60 border-gray-300/50 dark:border-slate-800/40 opacity-70'
                  }`}
                >
                  <div className="flex items-center justify-between border-b border-gray-200 dark:border-slate-800 pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        isRewardActive ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-gray-500/10 text-gray-400 border border-gray-500/20'
                      }`}>
                        {isRewardActive ? '🟢 Active' : '⚪ Deactive'}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        type="button"
                        onClick={() => handleToggleRewardActive(idx)}
                      ...
                      >
                        {isRewardActive ? 'Deactivate' : 'Activate'}
                      </button>
